...
export default {
    ...
    async read(collection, objFind, callback){
        let client;
        try {
            client = await MongoClient.connect(config.MongoURL,{ useNewUrlParser: true });
            const db = client.db(config.MongoDB);
            let r = await db.collection(collection).findOne(objFind, function(err, doc){
                if(!err && doc){
                    callback(false, doc);
                } else {
                    callback(err);
    ...
    async readAll(collection, objFind, callback){
        let client;
        try {
            client = await MongoClient.connect(config.MongoURL, { useNewUrlParser: true });
            const db = client.db(config.MongoDB);
            let r = await db.collection(collection).find(objFind).each(function(err, docs){
                if(!err && doc){
                    callback(false, docs);
                } else {
                    callback(err);
